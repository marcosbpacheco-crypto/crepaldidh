            ...
            <div className="flex flex-wrap gap-1.5">
              {NAV_ITEMS.map(item => (
                <button key={item.id} onClick={() => setTab(item.id)}
                  className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl text-[9px] sm:text-[10px] font-bold transition-all ${tab === item.id ? 'bg-gradient-to-r from-violet-600 to-purple-600 text-white shadow-sm' : 'bg-white text-slate-600 border border-slate-100 hover:border-violet-200 shadow-sm'}`}>
                  <item.icon className="w-3 h-3 sm:w-3.5 sm:h-3.5" /><span className="hidden xs:inline">{item.label}</span>
                </button>
              ))}
            </div>
