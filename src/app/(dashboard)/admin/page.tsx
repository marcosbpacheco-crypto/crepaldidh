...
const MODULE_LABELS: Record<ModuleName, string> = {
  ...
  cadastros: 'Cadastros Base', tasks: 'Tarefas', alerts: 'Alertas', import: 'Importação',
}
                  ...
                  <tr key={p.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                    ...
                    {(['canView', 'canCreate', 'canEdit', 'canDelete', 'canExport'] as const).map(field => (
                      <td key={field} className="px-4 py-2.5 text-center">
                        <button onClick={() => admin.checkPermission('admin', 'edit') && admin.updatePermission(p.id, field, !p[field])}
                          className={`w-6 h-6 rounded-lg flex items-center justify-center transition-colors ${p[field] ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-slate-50 text-slate-300 border border-slate-100 hover:border-slate-200'} ${!admin.checkPermission('admin', 'edit') ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'}`}>
