...
import { Target, Calendar, Clock, User, Building2, Star, CheckCircle, TrendingUp } from 'lucide-react'
...
export default function OverviewTab({ program }: { program: MentoringProgram }) {
  const { participants, sessions } = useMentoring()
  ...
  const avgSat = ratings.length ? (ratings.reduce((s, v) => s + v, 0) / ratings.length).toFixed(1) : '—'
  ...
  const statCards = [
    ...
    { label: 'Sessões', value: `${doneSessions}/${pSessions.length}` },
    { label: 'Horas de mentoria', value: hours.toFixed(1) },
    { label: 'Objetivos', value: `${doneObjectives}/${pObjectives.length}` },
    { label: 'Ações', value: `${doneActions}/${pActions.length}` },
    { label: 'Satisfação', value: avgSat },
  ]
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-4">
                <div className="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-violet-500 to-purple-500 rounded-full transition-all" style={{ width: `${mainObjective.progress || 0}%` }} />
                </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-lg font-bold text-slate-800">Informações</h3>
          </div>
          <div className="space-y-3">
            {[
              { icon: User, label: 'Mentorado', value: program.menteeName || '—' },
              { icon: Building2, label: 'Empresa', value: program.companyName || '—' },
              { icon: User, label: 'Mentor', value: program.mentor || '—' },
              { icon: Calendar, label: 'Início', value: program.startDate ? new Date(program.startDate).toLocaleDateString('pt-BR') : '—' },
              { icon: Calendar, label: 'Encerramento', value: program.endDate ? new Date(program.endDate).toLocaleDateString('pt-BR') : '—' },
              { icon: Building2, label: 'Cargo do mentorado', value: program.menteeRole || '—' },
              { icon: Building2, label: 'Departamento', value: program.menteeDepartment || '—' },
              { icon: Building2, label: 'Gestor', value: program.menteeGestor || '—' },
            ].map((row, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {program.mainObjective && (
          <div className="bg-gradient-to-br from-violet-50 to-purple-50 border border-violet-100 rounded-2xl p-6">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h4 className="text-sm font-bold text-violet-800">Objetivo Geral da Mentoria</h4>
            </div>
            <p className="text-sm text-violet-900">{program.mainObjective}</p>
          </div>
        )}
        ...
        <div className="bg-gradient-to-br from-blue-50 to-slate-50 border border-blue-100 rounded-2xl p-6">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h4 className="text-sm font-bold text-blue-800">Próxima Sessão</h4>
          </div>
          {nextSession ? (
            <div>
              ...
              <p className="text-sm text-slate-500 mt-1">
                ...
                {nextSession.startTime ? ` às ${nextSession.startTime}` : ''} · {nextSession.duration}min
              </p>
            </div>
          ) : (
